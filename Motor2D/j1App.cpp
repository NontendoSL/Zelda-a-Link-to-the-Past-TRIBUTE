...
bool j1App::Awake()
{
	...
	if(ret == true)
	{
		...
		{
			ret = item._Ptr->_Myval->Awake(config.child(item._Ptr->_Myval->name.GetString()));
			item++;
		}
	}
	...
	if(size > 0)
	{
		...
		if(result != NULL)
		{
			...
			if(ret == true)
				...
				LOG("...loading process interrupted with error on module %s", (item._Ptr->_Myval != NULL) ? item._Ptr->_Myval->name.GetString() : "unknown");//TODO LOW
		}
		else
...
bool j1App::SavegameNow() const
{
	...
	if(ret == true)
	{
		...
		LOG("Save process halted from an error in module %s", (item._Ptr->_Myval != NULL) ? item._Ptr->_Myval->name.GetString() : "unknown");
